...
- задание функции спроса с помощью коэффициентов $a$, $b$
- задание момента насыщения $tn$. При увеличении данного параметра возрастает общий объем заказа и потери. $T ≤ tn ≤ T1$
- задание момента продажи первой партии товара $T$. При увеличении данного параметра возрастает общий объем заказа и потери. $0 ≤ T ≤ tn$
- задание момента полной реализации товара $T1$. При увеличении данного параметра увеличивается объем первой партии и уменьшаются потери. $tn ≤ T1$
- задние цены закупки $c$. При уменьшении данного параметра прибыль увеличивается. $0 ≤ c ≤ p$
- задание цены для покупателя $p$. При увеличении данного параметра прибыль увеличивается. $с ≤ p$
- задание скидки при дефиците товара $s$. При уменьшении данного товара потери уменьшаются, что приводит к увеличению прибыли. $0 ≤ s$

по заданным параметрам функции спроса и моментам реализации модель вычисляет:

- объем первой партии $q0$ и общий объем заказа $Q$
- максимальный уровень дефицита $B$ и общие потери $V$
- максимальную возможную прибыль и моменты времени $T$, $T1$ при соблюдении, которых она будет достигнута.
  Вычисление производится с ограничениями $0 ≤ T1≤ tn ≤ T ≤T’$. Где $T′$ изначально заданное $T$.
"""
